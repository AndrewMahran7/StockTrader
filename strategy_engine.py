...
class ORBStrategy:
    ...
    def reset_for_new_day(self):
        ...
        self.save_state()
        print(f"Reset strategy state for new day: {current_date}")
    ...
    def on_bar(self, bar: Dict[str, Any]):
        ...
        try:
            ...
            if (self.range_set and not self.trade_taken and 
                ...
                if close_price > self.opening_high and close_price > self.ema50:
                    self.enter_long(close_price, local_time)
            
            # Exit logic (if in position)
            if self.trade_taken and self.entry_price is not None:
                if (close_price >= self.tp or close_price <= self.sl):
                    self.exit_position(close_price, local_time, "TP/SL hit")
            ...
            if (self.trade_taken and self.entry_price is not None and 
                ...
                self.exit_position(close_price, local_time, "End of day")
    ...
    def submit_buy(self, price: float) -> Tuple[Optional[str], Optional[float]]:
        """
        Submit buy order via Alpaca API
        ...
        try:
            ...
            max_shares = round((buying_power * 0.95) / price, 6)
            ...
            if self.log_trade_func:
                ...
                self.log_trade_func(
                    self.session_name, self.symbol, "buy", max_shares, price,
                    max_shares * price, buying_power, float(account_after.buying_power),
                    f"ORB Entry - Order ID: {order.id}"
                )
    ...
    def submit_sell(self, price: float) -> Tuple[Optional[str], Optional[float]]:
        """
        Submit sell order via Alpaca API
        ...
        try:
            ...
            if self.log_trade_func:
                ...
                account_after = self.api.get_account()
                self.log_trade_func(
                    self.session_name, self.symbol, "sell", shares, price,
                    shares * price, float(account.buying_power), float(account_after.buying_power),
                    f"ORB Exit - Order ID: {order.id}"
                )
